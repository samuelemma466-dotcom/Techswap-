  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
       <div className="bg-slate-900 rounded-2xl w-full max-w-5xl h-[85vh] flex flex-col md:flex-row overflow-hidden border border-slate-800 shadow-2xl relative">
          <button onClick={onClose} className="absolute top-4 right-4 z-20 bg-black/50 p-2 rounded-full hover:bg-white/10 text-white"><Icons.X className="w-5 h-5" /></button>
          ...
          <div className="w-full md:w-2/5 bg-black relative flex flex-col">
              ...
              <div className="flex-1 p-6 bg-slate-950 border-t border-slate-800">
                  <div className="flex items-center justify-between mb-4">
                      <div><p className="text-xs text-slate-500 uppercase font-bold">Seller Trust</p><div className="flex items-center gap-2"><span className="text-2xl font-bold text-white">{product.sellerTrustScore}%</span><Icons.Shield className="w-5 h-5 text-emerald-500" /></div></div>
                      <div className="text-right"><p className="text-xs text-slate-500 uppercase font-bold">Grade</p><span className="px-3 py-1 bg-slate-800 border border-slate-700 text-white rounded font-bold">{product.condition}</span></div>
                  </div>
                  <div className="bg-emerald-900/20 border border-emerald-500/20 p-3 rounded-lg flex gap-3 items-center"><Icons.CheckCircle className="w-5 h-5 text-emerald-500" /><p className="text-sm text-emerald-200">Verified by TrustScan AI</p></div>
              </div>
          </div>
          ...
          <div className="w-full md:w-3/5 bg-slate-900 p-8 overflow-y-auto">
              ...
              <div className="flex items-end gap-4 mb-8">
                  ...
                  {pulse && <span className={`text-sm font-bold flex items-center gap-1 ${pulse.priceTrend === 'rising' ? 'text-red-400' : 'text-emerald-400'}`}><Icons.TrendingUp className="w-4 h-4" /> Market Trend</span>}
              </div>

              {pulse ? (
                  <div className="bg-slate-950 border border-slate-800 rounded-xl p-6 mb-8 relative overflow-hidden">
                      <div className="absolute top-0 right-0 p-4 opacity-5"><Icons.Chart className="w-32 h-32" /></div>
                      <h4 className="flex items-center gap-2 text-indigo-400 font-bold mb-4"><Icons.Chart className="w-5 h-5" /> MarketPulse Analysis</h4>
                      <div className="flex gap-6 items-center">
                          <div className="relative w-20 h-20 flex items-center justify-center border-4 border-slate-800 rounded-full"><span className="text-xl font-bold text-white">{pulse.valueScore}</span></div>
