        ...
        <div className="flex justify-between items-center h-16">
          <div className="flex items-center cursor-pointer gap-2" onClick={() => setView(userRole === 'seller' ? 'dashboard-overview' : 'landing')}>
            <div className="w-8 h-8 bg-gradient-to-tr from-emerald-500 to-teal-400 rounded-lg flex items-center justify-center shadow-lg shadow-emerald-500/20">
              <Icons.Shield className="w-5 h-5 text-slate-900" strokeWidth={2.5} />
            </div>
          ...
          {userRole === 'buyer' && (
            <div className="hidden md:flex items-center space-x-1 bg-slate-900/50 p-1 rounded-xl border border-slate-800">
              <NavBtn active={currentView === 'market'} onClick={() => setView('market')} icon={Icons.Package} label="Marketplace" />
              <NavBtn active={currentView === 'trust-scan'} onClick={() => setView('trust-scan')} icon={Icons.Shield} label="TrustScan" />
            </div>
          )}
             ...
             {userRole !== 'guest' && (
                <>
                  ...
                  <button onClick={() => setView('cart')} className="relative p-2 text-slate-400 hover:text-white transition-colors">
                    ...
                    {cartCount > 0 && <span className="absolute top-1 right-1 w-2 h-2 bg-emerald-500 rounded-full"></span>}
                  </button>
                  {onLogout && <button onClick={onLogout} className="text-xs font-bold text-slate-400 hover:text-red-400 ml-2">EXIT</button>}
                </>
             )}
          </div>
        </div>
